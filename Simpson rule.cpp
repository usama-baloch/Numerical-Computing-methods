...
//	int j=1;
	for(int i=1;i<n;i++)
	{
		if(i%2!=0)  //calculating sum of odd indexes
		{
			sum+=y[i];
		}
	}
...
//	int j=1;
	for(int i=1;i<n;i++)
	{
		if(i%2==0)  //calculating sum of even indexes
		{
			sum+=y[i];
		}
	}
...
int main()
{
	...
	if(n<2)
	{
		cout<<"Please enter max number of x greater or equal to 2."<<endl;
		goto again;
	}
	h=(b-a)/n;
	...
	y0=Y_value[0];
	yn=Y_value[n];
	result=(h/3.0)*(y0+4*sum1+ 2*sum2+yn);
	cout<<endl<<"---------------------------------------"<<endl;
